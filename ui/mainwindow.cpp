...
MainWindow::MainWindow(QWidget *parent) :
    ...
    ToolNameLst::getinstance();
    init_toolList();
//    m_DirRead = "d:/";
//    m_DirSave = "d:/";
    m_DirRead = "./";
    m_DirSave = "./";
    m_jsonfile = "./flow_data_.json";
    //    mp_flowApp = new MainApp();
//    m_flow_data = new  std::map<int, ToolBase * >();
    MainApp::getInstance(); // & theMainApp;
    ...
    //--------------------------------------------------------------------------
    std::cout << "____ MainWindow :  init  finish. _" << std::endl;
    std::cout << "    |            |" << std::endl;
    std::cout << "    --------------------\n\n" << std::endl;
}
...
void MainWindow::init_toolList() {
    for (int i = 0; i < ToolNameLst::gettoolNum(); i++) {
        QListWidgetItem *lst1;
        lst1 = new QListWidgetItem(QIcon(":/images/data.jpg"), QString::fromStdString(ToolNameLst::getNames()[i]),
                                   this->ui->listWidget);
        //        this->ui->listWidget->addItem(    QString::fromStdString(nameInToolBoxset[i]));
...
void MainWindow::slot_popEditToolItem() {
    ...
    std::string InFlow_name = curLine.toStdString();
    std::string nameInToolBox;// = nameInFlows_to_nameInToolBox(InFlow_name);
    std::cout << "slot_popEditToolItem: InFlow_name = " << InFlow_name << "----> -->"
              << std::endl;

    int id_tool = -1;// = nameInFlows_to_idx__from_flow_list(InFlow_name);
    std::cout << "id_tool = " << id_tool << std::endl;

    switch (id_tool) {
        case 2: {
            //        UISourceFilePcd *dt = new UISourceFilePcd(this, this->mp_flowApp, InFlow_name);//QDialog
            //        dt->setWindowTitle(QString::fromStdString(InFlow_name));
...
void MainWindow::addToolProc_from_ui() {
    ...
    res_insert =     MainApp::getInstance()->addToolItem_from_ui(nums, toolItemName, timeIdStr, soleName
//            , m_flow_data
            );
    if (res_insert) {
        //添加到流程表
        if (0) this->ui->listWidget_2->addItem(QString::fromStdString(soleName));
        QListWidgetItem *lst5 = new QListWidgetItem(QIcon(":/images/printer.jpeg"), QString::fromStdString(soleName),
...
//---------------------------------------
void MainWindow::loadFlowData() {
    try {

        std::string readFile = m_jsonfile;//"./out.json";
        ...
        //this->mp_flowApp->m
        //update app
        bool res = MainApp::getInstance()->from_json(this->m_json
//                , m_flow_data
                );
        if (!res) {
        ...
        for (nlohmann::json::iterator it = this->m_json.begin(); it != this->m_json.end(); ++it) {
            ...
            if (0)
                this->ui->listWidget_2->addItem(
                        QString::fromStdString((*it)["InFlow_nameID"]));

            QListWidgetItem *lst5 = new QListWidgetItem(
                    QIcon(":/images/printer.jpeg"), QString::fromStdString((*it)["InFlow_nameID"]), this->ui->listWidget_2);
            this->ui->listWidget_2->insertItem(1, lst5);
        }
...
void MainWindow::saveFlowData() {
    try {
        std::string savefile = m_jsonfile;// "./out.json";
        //    save_json( m_json  ,m_jsonfile ) ;
        this->m_json = {};
        nlohmann::json *js = new nlohmann::json();
        //    this->mp_flowApp->to_json( js  );
//        return ;
        MainApp::getInstance()->to_json(this->m_json
//                , m_flow_data
                );//return ;
...
//remove tool
void MainWindow::on_pushButton_minusTool_clicked() {

}

//run flow
void MainWindow::on_pushButton_runFlow_clicked() {
//MainApp::getInstance()->flowData;
for( std::map<int, ToolBase *>::iterator it=MainApp::getInstance()->flowData->begin()
        ;it!=MainApp::getInstance()->flowData->end();it++ )
{
    it->second->run();
}
}
